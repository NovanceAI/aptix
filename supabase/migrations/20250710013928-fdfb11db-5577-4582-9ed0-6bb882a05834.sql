...
-- Update the get_user_role function if needed (it should work automatically)
-- No changes needed to the function as it just returns the role from profiles table

-- Update RLS policies to account for area_admin role
-- Update evaluation_criteria policies to allow area_admins to manage criteria in their areas
DROP POLICY IF EXISTS "Client admins can manage criteria in their client areas" ON public.evaluation_criteria;
...
DROP POLICY IF EXISTS "Client admins can view profiles in their client" ON public.profiles;
...
DROP POLICY IF EXISTS "Client admins can update profiles in their client" ON public.profiles;
...
DROP POLICY IF EXISTS "Client admins can create profiles for their client" ON public.profiles;
...
DROP POLICY IF EXISTS "Client admins can delete profiles in their client" ON public.profiles;

CREATE POLICY "Client admins can delete profiles in their client" 
ON public.profiles 
FOR DELETE 
USING (
  (get_user_role(auth.uid()) = 'client_admin' AND client_id = get_user_client_id(auth.uid()) AND id <> auth.uid()) OR
  get_user_role(auth.uid()) = 'super_admin'
);