-- Add area_id to profiles table
ALTER TABLE public.profiles 
ADD COLUMN area_id UUID REFERENCES public.areas(id) ON DELETE SET NULL;

-- Update the handle_new_user function to support area assignment based on email domain
...
BEGIN
  ...
  IF matching_client_id IS NOT NULL THEN
    -- Get the first area for this client as default (if any)
    SELECT id INTO default_area_id
    FROM public.areas
    WHERE client_id = matching_client_id
    ...
    VALUES (
      NEW.id,
      matching_client_id,
      default_area_id, -- Will be NULL if no areas exist yet
      user_email,
  ...
  ELSE
    ...
    -- Create profile as client admin (first user for this domain) - no area assignment
    INSERT INTO public.profiles (id, client_id, area_id, email, first_name, last_name, role)
    VALUES (
      NEW.id,
      new_client_id,
